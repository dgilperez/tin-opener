...
module TinOpener
  RSpec.describe DataFileProcessor do
    ...
    context "CSV" do
      ...
      it "extracts the headers format from the first row" do
        expected_headers = {
          ...
          horas_totales_disponibilidad_bicicletas_en_anclajes: "String",
      ...
      it "converts all the rows to an array of hashes with snake-cased keys" do
        expected_last_row = {
          ...
          horas_totales_disponibilidad_bicicletas_en_anclajes: "35.124,73",
    ...
    context "XLST" do
      let(:data_file) { File.open(File.join('spec', 'fixtures', 'data_sets', 'DeclaracionBienesPatrimoniales.xlsx'), 'r') }
      let(:expected_headers) {
        { fecha_de_declaracion: 'String', dni: 'String', apellido_1: 'String', apellido_2: 'String', nombre: 'String' }
      }

      it "extracts the headers format from the first row - only first workbook" do
        expect(data_file_processor.headers).to eq expected_headers
      end

      it "converts all the rows to an array of hashes with snake-cased keys - only first workbook" do
        expected_last_row = { fecha_de_declaracion: '2015-10-15', dni: '00000000X', apellido_1: 'Garcia', apellido_2: 'Ruiz', nombre: 'Pepe' }
        expect(data_file_processor.rows.size).to eq(1)
        expect(data_file_processor.rows.last).to eq(expected_last_row)
      end
    end
